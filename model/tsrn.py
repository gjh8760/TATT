...
class TPInterpreter(nn.Module):
    def __init__(
                self,
                text_embedding,             # text prior from TP Generator(=CRNN), 37
        ...
        self.fc_in = nn.Linear(text_embedding, d_model)
    ...
    def forward(self, image_feature, tp_input):
        """
        image_feature: LR image features.       [B, C_i, H_i, W_i] = [B, 64, 16, 64]
        tp_input: TP generator output.          [B, C_t, 1, N] = [B, 37, 1, 26]
        """
        ...
        x_tar = image_feature   # [B, 64, 16, 64]
        ...
        text_prior = text_prior.mean(0)             # batch 단위 mean??
...
class TSRN_TL_TRANS(nn.Module):
    def __init__(self,
                 ...
                 hidden_units=32,
                 word_vec_d=300,
                 text_embedding=37, #37, #26+26+1 3965        # 2354 for korean
    ...
    def forward(self, x, text_emb=None):
        ''' x: LR image with mask. [B, 4, 16, 64] '''

        # save!!!
        # torch.save(x, '0_LR.pt')
        # torch.save(text_emb, '1_TP.pt')

        if self.stn and self.training:
            _, ctrl_points_x = self.stn_head(x)
            x, _ = self.tps(x, ctrl_points_x)
        block = {'1': self.block1(x)}
        ''' block['1']: LR image feature (=f_I in paper). [B, 4, 16, 64] '''

        if text_emb is None:
            text_emb = torch.zeros(x.shape[0], self.text_embedding, 1, 26).to(x.device)
        ...
        output = torch.sigmoid(block[str(self.srb_nums + 3)])
        return output
...
class MyUpsampleBlock(nn.Module):
    """
    No activation function used in final upsampling block.
    """
    def __init__(self, in_channels, up_scale):
        super(MyUpsampleBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, in_channels * up_scale ** 2, kernel_size=3, padding=1)
        self.pixel_shuffle = nn.PixelShuffle(up_scale)
