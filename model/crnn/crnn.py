...
class BidirectionalLSTM(nn.Module):
    ...
    def forward(self, input):
        ...
        #     torch.save(param, name + '.pt')

        # torch.save(input, '0_input.pt') # ok!
        # torch.save(recurrent, '1_after_lstm.pt')    # ok!
        # torch.save(t_rec, '2_after_view.pt')    # ok!

        self.embedding = self.embedding.double()
        t_rec = t_rec.double()
        output = self.embedding(t_rec).float()  # [T * b, nOut]
        # save!!!
        # torch.save(output, '3_after_linear.pt')

        output = output.view(T, b, -1)

        # save!!!
        # torch.save(output, '4_after_view.pt')
...
class CRNN(nn.Module):
    ...
    def __init__(self, imgH=32, nc=1, nclass=37, nh=256, n_rnn=2, leakyRelu=False):
    ...
    def forward(self, input):
        ...
        # save!!!
        # torch.save(conv, '0_after_conv.pt')
        ...
        # save!!!
        # torch.save(conv, '0.5_before_rnn.pt')


        # rnn features
        output = self.rnn(conv)         # batch size 문제 이거 통과하면서 생김

        # save!!!
        # torch.save(output, '1_after_rnn.pt')
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride = (1,1), stage=-1):
        downsample = None
