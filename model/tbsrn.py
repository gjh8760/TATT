...
class FeatureEnhancer(nn.Module):
    ...
    def __init__(self):
        ...
        self.pe2d = positionalencoding2d(64,16,256).float()
    ...
    def forward(self, conv_feature):
        ...
        position2d = self.pe2d.to(conv_feature.device).unsqueeze(0).view(1,64,16 * 256) #positionalencoding2d(64,16,64).float().to(conv_feature.device)
...
class InfoGen(nn.Module):
    def __init__(
        ...
        self.tconv1 = nn.ConvTranspose2d(t_emb, 128, 3, 2, bias=False)
        self.bn1 = nn.BatchNorm2d(128)

        self.tconv2 = nn.ConvTranspose2d(128, 128, 3, 2, bias=False)
...
class TBSRN_TL(nn.Module):
    def __init__(self, scale_factor=2,
        ...
        self.srb_nums = srb_nums
        for i in range(srb_nums):
            setattr(self, 'block%d' % (i + 2), RecurrentResidualBlock_TL(2 * hidden_units, text_channels=out_text_channels))
    ...
    def forward(self, x, text_emb):
        ...
        spatial_t_emb, weights = self.infoGen(text_emb)
...
class RecurrentResidualBlock_TL(nn.Module):
    def __init__(self, channels, text_channels):
        super(RecurrentResidualBlock_TL, self).__init__()
        self.conv1 = nn.Conv2d(channels, channels, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(channels)
        self.gru1 = GruBlock(channels, channels)
        self.prelu = nn.ReLU()
        ...
        self.conv_proj = nn.Conv2d(channels + text_channels, channels, kernel_size=1, padding=0)
    ...
    def forward(self, x, text_emb):
        ...
        fused_feature = torch.cat([residual, text_emb], 1)
        residual = self.prelu(self.conv_proj(fused_feature))

        # print("residual:", residual.shape, x.shape)
